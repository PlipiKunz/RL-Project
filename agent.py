...
class Agent():
    ...
    def _build_model(self):
        model = Sequential()
        model.add(Input((5)))
        model.add(Dense(32,activation = 'relu', kernel_initializer = tf.keras.initializers.VarianceScaling(scale=2)))
    ...
    def get_action(self,state):
        ...
        """Do Best Acton"""
        a_index = np.argmax(self.model.predict(np.array(state).reshape(1,-1)))
    ...
    def learn(self,debug = False):
        ...
        labels = self.model.predict(x = np.array(states))
        next_state_values = self.model_target.predict(x = np.array(next_states))
        ...
        for i in range(size):
            ...
            truthyval =  (not next_done_flags[i]) 
            a = next_rewards[i]
            b = max(next_state_values[i])
            labels[i][action] = a + truthyval * self.gamma * b

        """Train our model using the states and outputs generated"""
        self.model.fit(np.array(states),labels,batch_size = size, epochs = 1, verbose = 0)
